...
export default function SimControl() {
  ...
  return (
    <div className="flex items-center gap-1.5 ml-auto">
      {/* Progress bar — only while playing or paused */}
      {state !== 'stopped' && (
        <div className="w-16 h-1.5 bg-gray-700 rounded-full overflow-hidden">
          <div className="h-full bg-shell-yellow rounded-full transition-all duration-500"
            style={{ width: `${pct}%` }} />
        </div>
      )}

      <span className={`text-xs font-mono w-12 text-right ${
        state === 'playing' ? 'text-green-400' :
        state === 'paused'  ? 'text-yellow-400' : 'text-gray-600'
      }`}>
      ...
      <button
        ...
        className={`p-1.5 rounded-lg transition-colors disabled:opacity-40 ${
          state === 'playing'
            ...
            : 'bg-green-900/60 text-green-400 hover:bg-green-800/60'
        }`}
      ...
      {state !== 'stopped' && (
        <button
          onClick={() => send('stop')}
          disabled={busy}
          className="p-1.5 rounded-lg bg-red-900/60 text-red-400 hover:bg-red-800/60 transition-colors disabled:opacity-40"
