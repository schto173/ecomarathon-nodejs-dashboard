  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 flex flex-col gap-4">
      <div className="flex items-center gap-2">
        ...
        {fuelMl != null && fuelMl > FUEL_LIMIT_ML * 0.9 && (
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div className="bg-gray-800 rounded-lg p-3">
          ...
          <div className="font-mono font-bold text-white">
            {avgFuelPerLap != null ? avgFuelPerLap.toFixed(1) : '—'}
            <span className="text-xs font-normal text-gray-400 ml-1">ml</span>
          </div>
        </div>
      </div>

      {projectedTotalFuel != null && (
        <div className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm ${
          projectedTotalFuel > FUEL_LIMIT_ML ? 'bg-red-900/40 text-red-300' : 'bg-green-900/30 text-green-300'
        }`}>
          <span className="text-xs">Projected finish</span>
          <span className="font-mono font-bold">{projectedTotalFuel.toFixed(0)} ml / {FUEL_LIMIT_ML} ml</span>
        </div>
      )}
    </div>
  )
